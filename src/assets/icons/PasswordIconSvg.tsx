import {SVGComponentPropType} from '@/utils/responsive';
import {Defs, G, LinearGradient, Path, Stop, Svg} from 'react-native-svg';

const PasswordIconSvg: React.FC<SVGComponentPropType> = ({
  width = 24,
  height = 24,
}) => {
  return (
    <Svg
      width={width}
      height={height}
      viewBox="0 0 24 24"
      fill="none">
      <G opacity="0.5">
        <Path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M7.7688 8.71381H16.2312C18.5886 8.71381 20.5 10.583 20.5 12.8885V17.8253C20.5 20.1308 18.5886 22 16.2312 22H7.7688C5.41136 22 3.5 20.1308 3.5 17.8253V12.8885C3.5 10.583 5.41136 8.71381 7.7688 8.71381ZM11.9949 17.3294C12.4928 17.3294 12.8891 16.9419 12.8891 16.455V14.2489C12.8891 13.7719 12.4928 13.3843 11.9949 13.3843C11.5072 13.3843 11.1109 13.7719 11.1109 14.2489V16.455C11.1109 16.9419 11.5072 17.3294 11.9949 17.3294Z"
          fill="url(#paint0)"
        />
        <Path
          opacity="0.4"
          d="M17.523 7.39595V8.86667C17.1673 8.7673 16.7913 8.71761 16.4052 8.71761H15.7447V7.39595C15.7447 5.37868 14.0681 3.73903 12.0053 3.73903C9.94257 3.73903 8.26594 5.36874 8.25578 7.37608V8.71761H7.60545C7.20916 8.71761 6.83319 8.7673 6.47754 8.87661V7.39595C6.4877 4.41476 8.95692 2 11.985 2C15.0537 2 17.523 4.41476 17.523 7.39595Z"
          fill="url(#paint1)"
        />
      </G>
      <Defs>
        <LinearGradient
          id="paint0"
          x1="3.5"
          y1="7.89799"
          x2="22.8361"
          y2="11.7312"
          gradientUnits="userSpaceOnUse">
          <Stop stopColor="#53E88B" />
          <Stop offset="1" stopColor="#15BE77" />
        </LinearGradient>
        <LinearGradient
          id="paint1"
          x1="6.47754"
          y1="1.57775"
          x2="18.773"
          y2="4.63763"
          gradientUnits="userSpaceOnUse">
          <Stop stopColor="#53E88B" />
          <Stop offset="1" stopColor="#15BE77" />
        </LinearGradient>
      </Defs>
    </Svg>
  );
};

export default PasswordIconSvg;
